...
17-Find-a-node-in-LL-(recursive)
...
int findANodeInLLRecursively(Node* head,int data){
	/* stopping the search mid way*/
	...
	int smallOutput = findANodeInLLRecursively(head -> next, data);
	if(smallOutput != -1){
...
int main(){
	...
	// call the function for calculating the length of the linked list
		...
		cout << findANodeInLLRecursively(head, data) << endl;
