...
You have been given a singly linked list of integers. Write a function that returns the index/position of an integer 
...
For the given singly linked list, considering the indices starting from 0, progressing in a left to right manner 
...
int findANodeInALinkedList(Node* head){
	
	// take the element which has to be found
	
	int x;
	cin >> x;
	int index = 0;
	
	// iterate and keep a count of the index and check the data // return accordingly
	Node* temp = head;
	while(temp != NULL){
		if(temp -> data == x){
			return index;
...
int main(){
	...
	// call the function for calculating the length of the linked list
		...
		cout << findANodeInALinkedList(head) << endl;
