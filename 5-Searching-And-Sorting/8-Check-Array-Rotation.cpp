...
void checkArrayRotation(int arr[], int size){
	
	// for loop with constant comparison for the bigger integer than the front
	
	for(int i = 0; i < size - 1; i++){		// size - 1 cause we are dealing with i + 1
		if(arr[i] > arr[i + 1]){
			cout << i + 1;				
			return;
		}
